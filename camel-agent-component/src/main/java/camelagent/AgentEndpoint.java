...
public class AgentEndpoint extends DefaultEndpoint {
	...
	public Matcher getBodyMatcher(String bodyContent) {
		if (this.match != null) {
			Pattern pattern = Pattern.compile(this.match + "(?:\\[.+\\])?"); // This appends an extra pattern to allow
																				// for Jason annotations on body content
	 ...
	 * @param matcher
	 * @param bodyContent
	 * @return String Match and replace message content, using the uri information
	 */
	public String getReplacedContent(Matcher matcher, String bodyContent) {
		...
		if (this.replace != null) {
			...
			/*
			 * String[] replaces = this.replace.split(","); StringBuilder sbuilder = new
			 * StringBuilder(); for (int i=0;i<replaces.length;i++) { Pattern pattern =
			 * Pattern.compile("\\$([1-9][0-9]*)"); Matcher m =
			 * pattern.matcher(replaces[i]); String replaceS = ""; if(m.matches()) { int g =
			 * Integer.parseInt(m.group(1)); replaceS = matcher.group(g);
			 * 
