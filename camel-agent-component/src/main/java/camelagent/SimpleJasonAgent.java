...
/**
 * @author surangika This class extends the logic in the SimpleJasonAgent class
 *         of the Jason distribution
 */
public class SimpleJasonAgent extends AgArch implements Serializable {
	 ...
	 * @param nafFunctors
	 * @param persistent
	 *            Updates the transient and persistent percepts using messages
	 *            received from the camel exchange
	 */
	public void updatePerceptList(Collection<Literal> newPercepts, String annotations, String updateMode,
	...
	private void updatePercepts(String updateMode, Set<String> nafFunctors, Collection<Literal> newPercepts,
			Queue<Literal> perceptQueue) {
		// If updateMode is "add" (default option) and an identical percept is not
		// already in the persistent queue, add the percept to the persistent queue
			...
			synchronized (perceptQueue) {
				...
				while (it.hasNext()) {
					...
					if (nafFunctors.contains(f) && !newPerceptFunctors.contains(f)) { // Negation as failure applies
						...
						perceptQueue.remove(lit); // Don't change to it.remove()!!! Can get IllegalStateException if lit
													// already removed, but "weakly consistent" iterator doesn't know.
					} else {
						// Negation as failure does not apply
						for (Literal newPercept : newPercepts) {
							if (f.equals(newPercept.getFunctor()) && lit.getArity() == newPercept.getArity()) {
			...
			if (m.matches()) {
				...
				if (newPercepts.size() > 1) {
					logger.log(Level.SEVERE,
							"Percept delete mode not compatible with non-singleton collection of percepts");
					return;
				}
				for (Literal l : newPercepts) { // Should have 0 or 1 iterations (not sure if 0 can occur)
					String[] argMatchMode;
					if (m.start(1) == -1) {
						// No arguments provided for 'delete'. Assume all args of queued percept must
						// match new percept for queued percept to be deleted
	...
	public List<Literal> perceive() {
		...
		// Remove transient percepts from the transient queue, and add them to agent's
		// internal percept list
		while (!temp_percepts.isEmpty()) {
			Literal li = temp_percepts.poll();
			l.add(li);
		}
		// Add persistent percepts to agent's internal percept list, but do not remove
		// them from the persistent queue
	...
	/**
	 * @param consumerType
	 * @return Returns the set of consumers for a given consumer type (messages or
	 *         actions)
	 */
