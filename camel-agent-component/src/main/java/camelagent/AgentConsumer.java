...
public class AgentConsumer extends DefaultConsumer {
	 ...
	 * @return Maps internal Jason agent actions to camel messages
	 */
	public Object agentInternallyActed(String agentName, String actionString, String actionName, List<Term> parameters,
			Unifier unifier) {
		...
		if (endpoint.getUriOption().contains("action")) {
			...
			try {
				...
				if (exchange.getException() != null) {
					...
					// Unification is applicable to InOut exchanges only. This waits for the return
					// message from the exchange
						...
						if (exchange.hasOut()) {
							...
							for (String[] mapping : mappings) {
								...
								// ArrayList<String> l =
								// (ArrayList<String>)exchange.getIn().getHeader(headerVal);
	...
	public boolean agentActed(JasonAction jasonAction) {
		...
		if (endpoint.getUriOption().contains("action")) {
			try {
				...
				sendActionToCamel(agName, actionTerm.toString(), actionTerm.getFunctor(), paramsAsStrings, exchange,
						annots);
		...
		if (endpoint.getUriOption().contains("message")) {
			try {
				...
				if (exchange.getException() != null)
					...
					String ep = endpoint.getExchangePattern().toString();
					if (ep.equals("InOut")) { /*
												 * if (exchange.getOut() != null) { Message m = new Message(); }
												 */
					}
				}
			}
	 ...
	 * @param messageBody
	 * @throws Exception
	 *             Sends out the camel message only if the uri information is
	 *             matched with the message content
	 */
	...
	/**
	 * @return List<String[]> Auxiliary method to interpret the result values sent
	 *         back for agent actions
	 */
	 ...
	 * @param annotations
	 * @throws Exception
	 *             Contains common logic to send internal and external actions as
	 *             camel messages
	 */
	private void sendActionToCamel(String agName, String actionString, String actionName, List<String> paramsAsStrings,
