...
/**
 * @author surangika Manages Jason agents in a Camel context
 */
public class AgentContainer {
	 ...
	 * @param nafFunctors
	 * @param persistent
	 *            receive the percepts from the camel exchange, and pass it to a
	 *            particular agent or all the agents, based on the value of the
	 *            receiver parameter
	 */
	public void getCamelpercepts(Collection<Literal> content, String receiver, String annotations, String updateMode,
		...
		try {
			...
			for (; it.hasNext();) {
				...
				if (!receiver.equals("")) {
					...
					// System.out.println("Updating percepts for agent " + a.getAgName() + ";
					// content = " + content);
					a.updatePerceptList(content, annotations, updateMode, nafFunctors, persistent);
				}
			}
	 ...
	 * @param receiver
	 * 
	 *            receive the messages from the camel exchange, and pass it to a
	 *            particular agent or all the agents, based on the value of the
	 *            receiver parameter
	 */
	public void getCamelMessages(Message message, String receiver) {
		try {
			...
			for (; it.hasNext();) {
				...
				// if the receiver value is "all", send the message to all the agents in the
				// context
	...
	private String qualifiedName(String containerId, String name) {
		// Prepend containerId if it has a non-empty name, indicating there may be
		// multiple containers
		if (containerId != "")
